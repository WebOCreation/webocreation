		...
		if (isset($data['newsticker_description'])) {
			foreach ($data['newsticker_description'] as $language_id => $value) {
				foreach ($value as $newsticker_description) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "newsticker_description SET newsticker_id = '" . (int)$newsticker_id . "', language_id = '" . (int)$language_id . "', message = '" .  $this->db->escape($newsticker_description['message']) . "', name = '" .  $this->db->escape($newsticker_description['name']) . "', position = '" .  (int)$newsticker_description['position'] . "'");
				}
			}
		}
