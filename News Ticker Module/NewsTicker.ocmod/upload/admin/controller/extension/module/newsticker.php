...
class ControllerExtensionModuleNewsticker extends Controller {
	...
	public function index() {
        if($this->checkDatabase()) {
            ...
            $data['install_database'] = $this->url->link('newsticker/install/installDatabase', 'token=' . $this->session->data['token'], 'SSL');
            ...
            $data['linktoken'] = $this->session->data['token'];
            ...
            if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
                if (!isset($this->request->get['module_id'])) {
                    $this->model_extension_module->addModule('newsticker', $this->request->post);
                } else {
            ...
            if (!isset($this->request->get['module_id'])) {
                $data['breadcrumbs'][] = array(
                    ...
                    'href' => $this->url->link('extension/module/newsticker', 'token=' . $this->session->data['token'], true)
                );
            } else {
                $data['breadcrumbs'][] = array(
                    'text' => $this->language->get('heading_title'),
                    'href' => $this->url->link('extension/module/newsticker', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], true)
                );
            }

            if (!isset($this->request->get['module_id'])) {
                $data['action'] = $this->url->link('extension/module/newsticker', 'token=' . $this->session->data['token'], true);
            } else {
                $data['action'] = $this->url->link('extension/module/newsticker', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], true);
            }
